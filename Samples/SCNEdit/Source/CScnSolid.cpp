...
{
    int n = 0;
    //sets read postion double the size of point3f and puts it in seekg
    celldata->bbsad = file->tellg();
    read_generic(celldata->bb_verts, file, sizeof(point3f) * 2);
    //reads file
...
log+=SAY("\n\t\t\tReading portal...");
    ...
    //reads the portals bb_verices in the file in double the size of point3f
    read_generic(portal->bb_verts,file,sizeof(point3f)*2);
    //creates new portal vertices
    portal->verts = new point3f[portal->n_verts];
    //reads the portal vertices in file in the size of point3f times the number of portal vertices
...
int CScnSolid::getCellAtPos(core::vector3df pos) const {
    f32 closest = 1000000000;
    u32 closestIndx = -1;
    for (u32 j = 0; j < n_cells; j++) {
        //Finds the closest cell to the position might not be perfectly accurate.
            f32 X = rawcells[j].bvh.bb_verts[0].x;
            f32 Y = rawcells[j].bvh.bb_verts[0].y;
            f32 Z = rawcells[j].bvh.bb_verts[0].z;
            f32 X2 = rawcells[j].bvh.bb_verts[1].x;
            f32 Y2 = rawcells[j].bvh.bb_verts[1].y;
            f32 Z2 = rawcells[j].bvh.bb_verts[1].z;
            if((X <= pos.X && Y <= pos.Y && Z <= pos.Z && X2 >= pos.X && Y2 >= pos.Y && Z2 >= pos.Z) ||
                (X >= pos.X && Y >= pos.Y && Z >= pos.Z && X2 <= pos.X && Y2 <= pos.Y && Z2 <= pos.Z)) {
                f32 dist = sqrtf(powf(X - pos.X,2) + powf(Y - pos.Y,2) + powf(Z - pos.Z,2));
                f32 dist2 = sqrtf(powf(X2 - pos.X, 2) + powf(Y2 - pos.Y, 2) + powf(Z2 - pos.Z, 2));
...
void CScnSolid::calcSurfVertices(u32 surfidx, f32* mults, IVertexBuffer* vbuff, u16 arg_alpha)
{
    ...
    for (u32 i=0; i < surfi->vertidxlen; i++)
    {
        ...
        tVert.TCoords2.X = uvpos[uvidxs[surfi->vertidxstart + i]].u;
        tVert.TCoords2.Y = uvpos[uvidxs[surfi->vertidxstart + i]].v;
        ...
        tVert.Color=video::SColor(a,255,255,255); //fixed (alpha should be set on all colors. Why?
