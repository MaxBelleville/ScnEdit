...
void CScnCellBBData::initMesh(CScn* scn)
{
	if (MeshBuffer != NULL)
		MeshBuffer->drop();
	...
	CMaterial* material =new CMaterial("portal", "BuiltIn/Shaders/Basic/Empty.xml");
	...
	for (u32 i = 0; i < scn->getSolidSize(0); i++) {
		...
		for (u32 j = 0; j < solid->n_cells; j++) {
			for (u32 l = 0; l < solid->rawcells[j].leafnode.size(); l++) {//TODO Abstract
				...
				core::vector3df start = leaf.bb_verts[0] +0.05; // Removes z-indexing
				core::vector3df end= leaf.bb_verts[1]-0.05; // at cost of accuracy
