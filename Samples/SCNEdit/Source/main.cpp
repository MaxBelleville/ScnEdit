    ...
    for (u32 i = 0; i < scn->getSolidSize(bLoadAll); i++) {
        ...
        for (u32 j = 0; j < solid->n_surfs; j++) {
            ...
            for (int k = 0; k < surfi->vertidxlen; k++) {
                //Gets vertex index
                point3f* verti = &solid->verts[solid->vertidxs[surfi->vertidxstart + k]];
                //Gets vector index
        ...
        for (u32 s = 0; s < scn->getSolidSize(bLoadAll); s++) {
            ...
            if (rs.size() > 0) {
                ...
                while (uvidxs.iterate(uvi)) {
                    ...
                    f32 OU = solid->ouvpos[uvi].u;
                    f32 OV = solid->ouvpos[uvi].v;
                    if (!uvresize) { //shift
                        U += uu;
                        V += vv;
                    }
                    ...
                    if (resetUV == true) {
                        ...
                        solid->uvpos[uvi].u = OU;
                        solid->uvpos[uvi].v = OV;
                    }
                    else {
                        solid->uvpos[uvi].u = U;
                        solid->uvpos[uvi].v = V;
...
void updateBufferUVfromScnSurf(u32 meshIndex, u32 si) {
        ...
        for (u32 i = 0; i < surfi->vertidxlen; i++) {
            ...
            verts[i].TCoords.X = uvi.u;
            verts[i].TCoords.Y = uvi.v;
            if (mults) {
                verts[i].TCoords2.X = (uvi.u * mults[0] + mults[2]);
                verts[i].TCoords2.Y = -(uvi.v * mults[1] + mults[3]);
            }
            else {
                verts[i].TCoords2.X = (uvi.u);
                verts[i].TCoords2.Y = -(uvi.v);
            }
...
bool scnSaveFile() {
    ...
    if (output&&output->is_open()) {
        for (u32 s = 0; s < scn->getSolidSize(bLoadAll); s++) {
            ...
            len = (solid->n_verts) * sizeof(point3f);
            output->write((char*)(solid->verts), len);


            //save uvpos
            output->seekp(solid->uvposad);
            len = (solid->n_uvpos) * sizeof(point2f);
        ...
        if (sels) {
            ...
            if (!(vert.x == 0 && vert.y == 0 && vert.z == 0)) {
                ...
                for (u32 j = 0; j < solid->n_cells; j++) {
                    ...
                    if (celldata) {
                        point3f v1 = celldata->bb_verts[0];
                        point3f v2 = celldata->bb_verts[1];//Update celldata
