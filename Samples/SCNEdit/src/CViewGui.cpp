...
CViewGui::CViewGui(CArguments* args)
{
	m_arguments = args;
...
void CViewGui::setupEvents() {
	...
	m_textSection1->OnTextSet = [interaction, isVertInf](UI::CUIBase* base) {
		...
		if (interaction->guiStateLayer().find(GUIState::EditFlags, GUIState::EditAlpha)) {
			...
			for (int i = 0; i < surfsels.size(); i++) {
				...
				CSolid* solid = scn->getSolid(surfsels[i].solididx);
				surf_t* surfi = solid->surfs[si];
			...
			if (interaction->guiStateLayer().get() == GUIState::EditSurfShading) {
				for (int i = 0; i < surfsels.size(); i++) {
					...
					{
						localVert_t* vert = &solid->local_faces[si].verts[j];
				...
				CSolid* solid = scn->getSolid(vertsel.solididx);

				localVert_t* vert = &solid->local_faces[vertsel.si].verts[vertsel.localidx];
	...
	interaction->OnKeyEvent([interaction, isVertInf, debug](key_pair pair) {
		...
		if (interaction->getKeyState(make_pair(KEY_KEY_C, KeyAugment::None))) {
			...
			CSolid* solid = scn->getSolid(surfdata.solididx);
			surf_t* surf = solid->surfs[surfdata.si];
		...
		if (interaction->getKeyState(make_pair(KEY_KEY_C, KeyAugment::Ctrl))) {
			...
			CSolid* solid = scn->getSolid(surfdata.si);
			surf_t* surf = solid->surfs[surfdata.si];
			if (surf->shading && surf->hasVertexColors) {
				if (interaction->guiStateLayer().swap(GUIState::Default, GUIState::EditFlags,
...
void CViewGui::onGUI()
{
	...
	if (scn) {
		...
		if (cellindx >= 0) {
			rawCell_t* cell = scn->getSolid(0)->rawcells[cellindx];
	...
	if (ImGuiFileDialog::Instance()->Display("ChooseTEXT")) {
		if (ImGuiFileDialog::Instance()->IsOk()) { // action if OK
			...
			if (selected) {
				...
				CSolid* solid = SCNEdit::getSCN()->getSolid(meshcomp->getSolidIdx());
...
InfoText CViewGui::getSolidInfo(bool inVertInf) {
	...
	CSolid* solid = scn->getSolid(surfdata.solididx);
	surf_t* surf = solid->surfs[surfdata.si];
	projBasis_t* projBasis = solid->projection[surfdata.si];
	plane_t* plane = solid->planes[surf->planeidx];
	...
	if (!inVertInf || interaction->keyAugLayer().get() == KeyAugment::None) {
		...
		vertBox_t vertsel = interaction->getVertSelected();
		CSolid* solidv = scn->getSolid(vertsel.solididx);
		surf_t* surfv = solidv->surfs[vertsel.si];
		projBasis_t* projBasisV = solidv->projection[vertsel.si];

		localVert_t vert = solidv->local_faces[vertsel.si].verts[vertsel.localidx];
	...
	if (interaction->getKeyState(make_pair(KEY_TAB, KeyAugment::None))) {
		plane_t* surfPlane = solid->planes[surf->planeidx];
		...
		s16 nodeindx = solid->tree->findNodePos(solid->planes, pushIn);
		node_t* node = solid->tree->nodes[nodeindx];
		...
		if (node->cell >= 0) {
			rawCell_t* cell = solid->rawcells[node->cell];
		...
		info.section2 = format("material = {} | material flags = {} | no special geom | unk = {} |",
			getMaterialName(node->material), node->material, node->visframe);
		if (node->specialGeomIdx >= 0 && node->specialGeomIdx < solid->n_names) {
			if (!solid->names[node->specialGeomIdx]) {
				info.section2 = format("material ={} | material flags ={} | special geom = {} | unk = {} |",
					getMaterialName(node->material), node->material & 0x0F,
	...
	if (interaction->getKeyAugment(KeyAugment::CtrlShift)) {
		if (surf->solidref_index >= 1) {
			CEnt* ent = scn->getSolidRef(surf->solidref_index);
...
InfoText CViewGui::getPortalInfo() {
	...
	CSolid* solid = scn->getSolid(0);
	rawCell_t* cell = solid->rawcells[portaldata.cellidx];
	portal_t* portal = solid->rawcells[portaldata.cellidx]->portals[portaldata.portalidx];
	rawCell_t* nextcell = solid->rawcells[portal->nextcell];
	plane_t plane = portal->plane;

	info.section1 = format(" | portal id: {} name: \"{}\" | cell #{}, name: \"{}\" | nextcell id: {}, name: \"{}\" "
		"| normal = ({:.1f}, {:.1f}, {:.1f}), d = {:.1f} | unknown= {} {} {:.2f} |",
		portaldata.portalidx, portal->name, portaldata.cellidx, cell->name, portal->nextcell,
		nextcell->name, plane.a, plane.b, plane.c, plane.d, portal->flag1, portal->flag2, portal->winding);
	return info;
}
