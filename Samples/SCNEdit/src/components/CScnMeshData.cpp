...
void CScnMeshData::initMesh(CSolid* mesh, CLightmap* lmap, CArguments* args)
{
	if (MeshBuffer != NULL)
		MeshBuffer->drop();
	...
	{
		CLocalizedFace* local = &mesh->local_faces[i];
...
surfaceBox_t CScnMeshData::collectSurfFromTri(CSolid* solid, core::triangle3df tri) {
	core::vector3df point[3] = { tri.pointA, tri.pointB, tri.pointC };

	int selsurf = -1;

	for (int i = 0; i < solid->n_surfs; i++) {
		surf_t* surfi = solid->surfs[i];
...
core::array<surfaceBox_t> CScnMeshData::select(CSolid* solid, core::triangle3df tri, bool bAdd) {
...
void CScnMeshData::deselect(CSolid* solid, int si) {
	if (si == -1) {
		for (int i = 0; i < surfsels.size(); i++)
...
void CScnMeshData::selectMat(CSolid* solid) {
	for (int i = 0; i < surfsels.size(); i++) {
		// regular surfs
...
void CScnMeshData::deselectMat(CSolid* solid, int si) {
	CMaterial* mat = RenderMesh->Materials[si];
...
void CScnMeshData::hide(CSolid* solid, bool bShared) {
	for (int i = 0; i < surfsels.size(); i++) {
		// regular surfs
		int si = surfsels[i].si;
...
void CScnMeshData::setTexture(CSolid* solid, const char* path) {
	for (int i = 0; i < surfsels.size(); i++) {
		int si = surfsels[i].si;
...
void CScnMeshData::updatePlane(CSolid* solid, int si) {
	IVertexBuffer* vb = RenderMesh->getMeshBuffer(si)->getVertexBuffer();
	IIndexBuffer* ib = RenderMesh->getMeshBuffer(si)->getIndexBuffer();
	...
	plane_t* planei = solid->planes[surfi->planeidx];
	planei->a = avg.X;
	planei->b = avg.Y;
	planei->c = avg.Z;
	planei->d = d;
...
void CScnMeshData::updateVert(CSolid* solid, vertBox_t vertsel, core::vector3df add) {
...
void CScnMeshData::resetVert(CSolid* solid, vertBox_t vertsel) {
	localVert_t* vert = &solid->local_faces[vertsel.si].verts[vertsel.localidx];
...
void CScnMeshData::updateUV(CSolid* solid, UVMode mode, core::vector2df uvAdd) {
	for (u32 i = 0; i < surfsels.size(); i++) {
		...
		projBasis_t* proj = solid->projection[si];
		...
		for (int f = 0; f < surfi->faceidxlen; f++) {
			localVert_t* vert = &solid->local_faces[si].verts[f];
			...
			if (mode == UVMode::Move) {
				...
				vert->uv *= (core::vector2df(1.0f) - uvAdd);
			}
			else if (mode == UVMode::FlipH) { // FLIP H
...
void CScnMeshData::resetUV(CSolid* solid) {
	for (int i = 0; i < surfsels.size(); i++) {
		int si = surfsels[i].si;
		...
		projBasis_t* paramFrame = solid->projection[si];
