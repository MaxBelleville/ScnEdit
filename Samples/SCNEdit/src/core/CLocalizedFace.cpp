...
CLocalizedFace::~CLocalizedFace() {
	...
	for (u32 i = 0; i < verts.size(); i++) {
		for (u32 j = 0; j < verts[i].shared.size(); j++) {
			localVert_t* peer = verts[i].shared[i];
			if (peer) {
...
void CLocalizedFace::calcVertices(u32 alpha, u16 hasVertexColors, IVertexBuffer* vbuff) {
...
void CLocalizedFace::calcIndices(IIndexBuffer* ibuff) {
	ibuff->addIndex(0);
	ibuff->addIndex(1);
	ibuff->addIndex(2);
...
core::vector2df CLocalizedFace::projectVertToUv(u16 localidx, projBasis_t* proj, u16 width, u16 height) {
...
core::vector3df CLocalizedFace::getVertCenter() {
...
std::pair<int, float> CLocalizedFace::getNearestVert(core::vector3df pos) {
	int nearestIdx = -1;
...
bool CLocalizedFace::isVertShared(u16 localidx) {
	for (u32 i = 0; i < verts[localidx].shared.size(); i++) {
		localVert_t* peer = verts[localidx].shared[i];
		if (peer) {
...
int CLocalizedFace::findVertFromGlobal(u32 faceidx) {
	for (u32 i = 0; i < verts.size(); i++) {
