//To understand this file you need to learn export.h
//util.h and irrlicht.h
#include "pch.h"
#include "include/io/export.h"
#include "include/util.h"
using namespace irr;
using namespace core;
void scnExportObj(CScn* scn, const char* name)
{
	u32 i;
	char tmp[16];
	char objname[256];
	char mtlname[256];
	sprintf_s(objname, "%s.obj", name);
	sprintf_s(mtlname, "%s.mtl", name);

	os::Printer::log("Building obj and mtl file...");
	FILE* obj;
	fopen_s(&obj, objname, "wt");
	if (obj == NULL)
		error(true, "Can't write to file %s\n", obj);
	FILE* mtl;
	fopen_s(&mtl, mtlname, "wt");
	if (mtl == NULL)
		error(true, "Can't write to file %s\n", obj);

	fprintf(obj, "mtllib %s\n", mtlname);

	u32 nvp = 0; //number of vertices in previous solid
	u32 nuvp = 0; //number of uvpos in previous solid

	for (u32 idx = 0; idx < scn->header->n_solids; idx++)
	{
		CSolid* solid = scn->getSolid(idx);

		u32 n_surfs = solid->n_surfs;
		u32 n_verts = solid->n_verts;
		u32 n_uvpos = solid->n_uvpos;

		sprintf_s(tmp, "g solid%03u\n", idx);
		fprintf(obj, tmp);
		for (i = 0; i < n_surfs; i++)
		{
			if (solid->surfs[i]->alpha <= 1)   //don't draw transparent solids
				continue;

			fprintf(obj, "usemtl %s\n", solid->surfs[i]->texture);
			fprintf(obj, "f ");
			u32 vstart = solid->surfs[i]->faceidxstart;

			for (u16 j = 0; j < solid->surfs[i]->faceidxlen; j++)
				fprintf(obj, "%u/%u ", solid->vertidxs[vstart + j] + 1 + nvp, solid->uvidxs[vstart + j] + 1 + nuvp);

			fprintf(obj, "\n");
		}

		for (i = 0; i < n_verts; i++) {
			core::vector3df* verti = &solid->verts[i];
			fprintf(obj, "v %f %f %f\n", verti->X, verti->Y, verti->Z);
			nvp++;
		}

		for (i = 0; i < n_uvpos; i++) {
			core::vector2df* uvi = &solid->uvpos[i];
			fprintf(obj, "vt %f %f \n", uvi->X, -uvi->Y);
			nuvp++;
		}

		for (i = 0; i < solid->textures.size(); i++)
		{
			fprintf(mtl, "newmtl %s\n", solid->textures[i].c_str());
			fprintf(mtl, "map_Kd textures/%s.bmp\n", solid->textures[i].c_str());
		}
	}

	fclose(obj);
	fclose(mtl);
	os::Printer::log("\tdone.");
}

void scnExport3ds(CScn* scn, const char* name)
{
	os::Printer::log("Building 3ds file...");

	char fname[128];
	sprintf_s(fname, "%s.3ds", name);
	FILE* f3ds = NULL;
	fopen_s(&f3ds, fname, "wt");
	if (f3ds == NULL)
		error(true, "Can't write to file %s\n", f3ds);

	for (u32 s = 0; s < scn->getSolidSize(true); s++) {
		CSolid* solid = scn->getSolid(s);
		u16 id;
		u32 size;
		u16 n_verts = solid->n_verts;
		u32 verts_size = n_verts * sizeof(core::vector3df);

		u16 n_surfs = solid->n_surfs;
		u16 n_polys = 0;
		for (int i = 0; i < n_surfs; i++)
			n_polys += (solid->surfs[i]->faceidxlen - 2);
		u32 polys_size = n_polys * 8;

		u32 len = 0;
		u8* data;
		u8* p;     //byte pointer

		len += 2 + 4; //main chunk include
		len += 2 + 4; //edit3ds chunk include
		len += 2 + 4; //edit_oject chunk include
		len += 2 + 4; //OBJ_TRIMESH chunk include
		len += verts_size + 2 + 2 + 4;       //vertex list include + data
		len += polys_size + 2 + 2 + 4;

		data = (u8*)malloc(len);
		p = data;
		id = MAIN3DS;
		size = len;

		memcpy(p, &id, 2); p += 2;
		memcpy(p, &size, 4); p += 4;

		id = EDIT3DS;
		size -= 6;

		memcpy(p, &id, 2); p += 2;
		memcpy(p, &size, 4); p += 4;

		id = EDIT_OBJECT;
		size -= 6;

		memcpy(p, &id, 2); p += 2;
		memcpy(p, &size, 4); p += 4;

		id = OBJ_TRIMESH;
		size -= 6;

		memcpy(p, &id, 2); p += 2;
		memcpy(p, &size, 4); p += 4;

		id = TRI_VERTEXL;
		size = verts_size + 2 + 6;

		memcpy(p, &id, 2); p += 2;
		memcpy(p, &size, 4); p += 4;
		memcpy(p, &n_verts, 2); p += 2;
		memcpy(p, solid->verts, verts_size); p += verts_size;

		id = TRI_FACEL1;
		size = polys_size + 2 + 6;
		memcpy(p, &id, 2); p += 2;
		memcpy(p, &size, 4); p += 4;
		memcpy(p, &n_polys, 2); p += 2;

		for (int s = 0; s < n_surfs; s++)
		{
			surf_t* surfi = solid->surfs[s];
			u16 vstart = surfi->faceidxstart;
			u16 idx;
			u16 vis = 6;
			idx = vstart;
			memcpy(p, &idx, 2); p += 2;
			idx = vstart + 1;
			memcpy(p, &idx, 2); p += 2;
			idx = vstart + 2;
			memcpy(p, &idx, 2); p += 2;
			memcpy(p, &vis, 2); p += 2;

			for (u16 j = 3; j < surfi->faceidxlen; j++)
			{
				idx = vstart + j - 1;
				memcpy(p, &idx, 2); p += 2;
				idx = vstart + j;
				memcpy(p, &idx, 2); p += 2;
				idx = vstart;
				memcpy(p, &idx, 2); p += 2;
				memcpy(p, &vis, 2); p += 2;
			}
		}
		fwrite(data, 1, len, f3ds);
	}
	fclose(f3ds);
	os::Printer::log("\tdone.");
}

void writeMapLine(FILE* map, vector3df a, vector3df b, vector3df c, vector3df t1, vector3df t2, const char* texname = 0)
{
	//FIX: fix direction of vertex, i just put - so it works
	fprintf(map, "( %.2f %.2f %.2f ) ", a.X, a.Z, a.Y);
	fprintf(map, "( %.2f %.2f %.2f ) ", b.X, b.Z, b.Y);
	fprintf(map, "( %.2f %.2f %.2f ) ", c.X, c.Z, c.Y);
	if (texname)
		fprintf(map, "%s ", texname);
	else
		fprintf(map, "NONE ");
	fprintf(map, "[ %.0f %.0f %.0f 0 ] [ %.0f %.0f %.0f 0 ] 0 1 1 \n",
		t1.X, t1.Z, t1.Y, t2.X, t2.Z, t2.Y);
}

void scnExportMap(CScn* scn, const char* name)
{
	// added to give feedback about export and match others -SJ
	os::Printer::log("Building map file...");

	char fname[128];
	sprintf_s(fname, "%s.map", name);
	FILE* map;
	fopen_s(&map, fname, "wt");
	fprintf(map,
		"{\n"
		"\"classname\" \"worldspawn\"\n"
		"\"mapversion\" \"220\"\n"
		"\"wad\" \"wads\\sample.wad\"\n");

	vector3df a, b, c, dp, r1, r2, normal, lastr1, zero(0, 0, 0);
	for (u32 idx = 0; idx < scn->header->n_solids; idx++)
	{
		CSolid* solid = scn->getSolid(idx);

		for (u32 i = 0; i < solid->n_surfs; i++)
		{
			surf_t* surfi = solid->surfs[i];
			u32 idxs[3];
			idxs[0] = surfi->faceidxstart;
			idxs[1] = surfi->faceidxstart + 1;
			a = solid->verts[solid->vertidxs[idxs[0]]];
			b = solid->verts[solid->vertidxs[idxs[1]]];

			f32 width = 0.5;
			plane_t* plane = solid->planes[surfi->planeidx];
			normal = vector3df(plane->a, plane->b, plane->c);
			dp = normal * width;

			r1 = (b - a);
			r2 = r1.crossProduct(normal);
			c = a + r2;
			r1.normalize();
			r2.normalize();

			fprintf(map, "{\n");
			writeMapLine(map, c, b, a, r1, r2, surfi->texture); //plane A
			writeMapLine(map, a - dp, b - dp, c - dp, r1, r2, surfi->texture); //plane B

			lastr1 = zero;
			u32 j = 0;
			do
			{
				idxs[0] = surfi->faceidxstart + j;

				j++;
				if (j == surfi->faceidxlen) j = 0;

				idxs[1] = surfi->faceidxstart + j;

				a = solid->verts[solid->vertidxs[idxs[0]]];
				b = solid->verts[solid->vertidxs[idxs[1]]];
				c = b - dp;
				r1 = (b - a);
				if (lastr1 != zero) {
					if (r1.crossProduct(lastr1).equals(zero)) //if lastr1 and r1 are collinear skip this plane (we already have it)
						continue;
				}

				r2 = r1.crossProduct(normal);
				r1.normalize();
				r2.normalize();
				writeMapLine(map, c, b, a, r1, r2); //plane C...

				lastr1 = r1;
			} while (j > 0);

			fprintf(map, "}\n");
		}
	}
	//os::Printer::log("Exported %d surfaces.\n",ns);
	fprintf(map, "}\n");

	//export entities
	CEnt* ent;
	u32 nents = scn->getTotalEnts();
	for (u32 i = 0; i < nents; ++i)
		ent = scn->getEnt(i);

	fclose(map);

	os::Printer::log("\tdone.\n\nAll done exporting. This process is not perfect so expect geometry glitches");
}