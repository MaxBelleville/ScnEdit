...
void write_generic(std::ofstream* file, void* buffer, int nbytes)
{
	...
	error(true, "write_generic: Error reading %i bytes", nbytes);
}
...
bool copy_portion(const char* existfile, const char* newfile, int64_t start, int64_t end)
{
	...
	while (bytes_to_copy > 0) {
		...
		read_generic(&ifs, buffer, static_cast<int>(chunk));
		write_generic(&ofs, buffer, static_cast<int>(chunk));
		bytes_to_copy -= chunk;
	}
...
core::array<std::string> search_dir_recursive(const std::string& dir, const std::string& target) {
	core::array<std::string> found;
	if (!std::filesystem::exists(dir) || !std::filesystem::is_directory(dir)) {
		return found;
	}

	std::string targetLower = toLowerStr(target);

	for (const auto& entry : std::filesystem::recursive_directory_iterator(dir)) {
		if (entry.is_regular_file()) {
			std::string fileName = entry.path().stem().string();

			// Check if the filename (without path) matches our target
			if (toLowerStr(fileName) == targetLower) {
...
const char* getMaterialName(u8 material) {
	...
	case 0x10:  return "Liquid";
	case 0x20:  return "Mud";
	case 0x30:  return "Gravel";
	case 0x40:  return "Plaster";
	case 0x50:  return "Carpet";
	case 0x60:  return "Glass";
	case 0x70:  return "Wood";
	case 0x80:	return "Creakwood";
	case 0x90:  return "Brick";
	case 0xA0:	return "Sheet Metal";
	case 0xB0:  return "Steel";
